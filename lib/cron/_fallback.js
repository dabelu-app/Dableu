...
async function getUserContact(userId) {
  try {
    ...
    return {
      ...
      notifyChannel: (f.notifyChannel?.stringValue || '').toLowerCase()  // 'whatsapp' | 'email' | ''
    };
  } catch (e) { return { chatId: '', email: '', name: '', notifyChannel: '' }; }
}
...
// נקרא כשאין Push. לפי בחירת המשתמש (notifyChannel): וואטסאפ או מייל.
// אם לא בחר — ברירת מחדל וואטסאפ קודם, אחרת מייל. מחזיר את הערוץ שנשלח.
async function fallbackNotify(userId, title, body) {
  const c = await getUserContact(userId);
  const msg = title + (body ? '\n\n' + body : '');
  const wa    = async () => (c.chatId && await sendWhatsApp(c.chatId, msg)) ? 'whatsapp' : '';
  const email = async () => (c.email  && await sendEmail(c.email, title, body || title)) ? 'email' : '';

  if (c.notifyChannel === 'email') {
    return (await email()) || (await wa()) || 'none';     // העדיף מייל, אחרת וואטסאפ
  }
  if (c.notifyChannel === 'whatsapp') {
    return (await wa()) || (await email()) || 'none';     // העדיף וואטסאפ, אחרת מייל
  }
  // ברירת מחדל ללא בחירה
  return (await wa()) || (await email()) || 'none';
}
