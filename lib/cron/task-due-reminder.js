...
module.exports = async (req, res) => {
  ...
  for (const user of users) {
    ...
    // משימות: שולחים תמיד — אם אין משימות פתוחות, שולחים הודעה מתאימה
    let pushTitle, pushBody;
    if (due.length && overdue.length) {
      pushTitle = `📅 משימות יומי (${due.length}+${overdue.length} ישנות)`;
      const dueList = due.slice(0,5).map((t,i)=>`${i+1}. ${t}`).join('\n');
      const oldList = overdue.slice(0,5).map((t,i)=>`${i+1}. ${t}`).join('\n');
      pushBody = `🔵 להיום:\n${dueList}\n\n⚠️ לא טופלו 48ש'+:\n${oldList}`.slice(0, 250);
    } else if (due.length) {
      pushTitle = `📅 משימות להיום (${due.length})`;
      pushBody  = due.map((t, i) => `${i + 1}. ${t}`).join('\n').slice(0, 200);
    } else if (overdue.length) {
      pushTitle = `⚠️ ${overdue.length} משימות באיחור`;
      pushBody  = overdue.map((t, i) => `${i + 1}. ${t}`).join('\n').slice(0, 200);
    } else if (openCount > 0) {
      pushTitle = `📋 ${openCount} משימות פתוחות`;
      pushBody  = 'אין משימות להיום — יש לך משימות מתוכננות לימים הקרובים 👍';
    } else {
