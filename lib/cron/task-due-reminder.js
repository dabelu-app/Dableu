...
// ── שלח Push לכל המכשירים של userId; אם אין Push — fallback לוואטסאפ/מייל ──
async function sendPush(userId, title, body, allowFallback = true) {
  if (!userId) return 0;
...
module.exports = async (req, res) => {
  ...
  // ── שלוף כל המשתמשים ──
  const uResp  = await fsFetch(
    `/users&pageSize=200`
  );
  const uData  = await uResp.json();
  let users  = (uData.documents || []).map(d => ({
    id:    d.name?.split('/').pop() || '',
    email: d.fields?.email?.stringValue || ''
  })).filter(u => u.id);

  // מצב בדיקה: ?uid=XXX → שליחה רק למשתמש אחד (לא שידור לכולם)
  if (req.query && req.query.uid) users = users.filter(u => u.id === req.query.uid);
  ...
  for (const user of users) {
    ...
    if (openCount > 0) {
      ...
      const parts = [];
      if (overdue.length) parts.push(`⚠️ באיחור:\n${overdue.slice(0, 5).map((t, i) => `${i + 1}. ${t}`).join('\n')}`);
      if (due.length)     parts.push(`📅 להיום:\n${due.slice(0, 6).map((t, i) => `${i + 1}. ${t}`).join('\n')}`);
      pushBody = (parts.join('\n\n') || 'אין משימות להיום — הכל מתוכנן לימים הקרובים 👍').slice(0, 800);
    } else {
      pushTitle = '✅ אין משימות פתוחות';
      pushBody  = 'הכל מטופל — יום מצוין! 💪';
    }
    // fallback לוואטסאפ/מייל רק כשיש משימות אמיתיות (להיום/באיחור) — לא על "אין משימות"
    const _meaningful = (due.length > 0 || overdue.length > 0);
    const sent = await sendPush(user.id, pushTitle, pushBody, _meaningful);
    if (sent > 0) notified++;
  }
  ...
  // ══════════════════════════════════════════════════════
  const stResp = await fsFetch(
    `/sharedTasks&pageSize=200`
  );
  const stData = await stResp.json();
  // משימות פתוחות שהן להיום או באיחור (תאריך יעד שעבר)
  const sharedRelevant = (stData.documents || []).filter(d => {
    const f   = d.fields || {};
    const due = f.dueDate?.stringValue || '';
    return f.status?.stringValue === 'pending' && due && due <= today;
  });

  // קבץ לפי עובד — בנפרד "להיום" ו"באיחור"
  const byWorker = {}; // email -> { today:[], overdue:[] }
  for (const doc of sharedRelevant) {
    const f             = doc.fields;
    ...
    if (!byWorker[assigneeEmail]) byWorker[assigneeEmail] = { today: [], overdue: [] };
    if (due === today)     byWorker[assigneeEmail].today.push(title);
    else if (due < today)  byWorker[assigneeEmail].overdue.push(title);
  }

  for (const [email, g] of Object.entries(byWorker)) {
    const worker = userByEmail[email];
    if (!worker) continue;
    const total = g.today.length + g.overdue.length;
    if (!total) continue;
    const pushTitle = g.overdue.length
      ? `✅ ${total} משימות פתוחות · ${g.overdue.length} באיחור`
      : `📅 ${g.today.length} משימות להיום`;
    const parts = [];
    if (g.overdue.length) parts.push(`⚠️ באיחור:\n${g.overdue.slice(0, 5).map((t, i) => `${i + 1}. ${t}`).join('\n')}`);
    if (g.today.length)   parts.push(`📅 להיום:\n${g.today.slice(0, 6).map((t, i) => `${i + 1}. ${t}`).join('\n')}`);
    const sent = await sendPush(worker.id, pushTitle, parts.join('\n\n').slice(0, 300));
    if (sent > 0) notified++;
  }
