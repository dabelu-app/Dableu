...
module.exports = async (req, res) => {
  ...
  // הערה: תזכורת על פגישות של "מחר" הוסרה לבקשת המשתמשת — אין יותר התראת יום-לפני לפגישות.
  // נשארו: סיכום הפגישות של היום (חלק 1) + "שעה לפני" (reminder-hourly).

  return res.status(200).json({ ok: true, sent, todayTotal: todayAppts.length });
};
