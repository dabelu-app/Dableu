...
module.exports = async (req, res) => {
        ...
        structuredQuery: {
          ...
          where: {
            compositeFilter: {
              op: 'AND',
              filters: [
                { fieldFilter: { field: { fieldPath: 'type' }, op: 'EQUAL', value: { stringValue: 'reminder' } } },
  ...
  for (const item of dueSoon) {
    ...
    const pushSent = await sendPush(userId,
      `⏰ תזכורת בעוד שעה`,
    ...
    if (pushSent > 0 || userId) {
      ...
      await fetch(
        `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT}/databases/(default)/documents/appointments/${docId}?updateMask.fieldPaths=hourBeforeSent&key=${FIREBASE_API_KEY}`,
