...
// ── דוח המייל למשרד ──
function reportHtml(clientName, office, p, consentAt, photoSaved) {
  ...
  rows += head('הסכמות')
    ...
    + row('צילום ת.ז.', photoSaved
        ? '<b style="color:#16a34a">✓ צורף</b> (בכרטיס הלקוח במערכת)'
        : '<b style="color:#dc2626">⚠ לא נשמר</b> — יש לבקש מהלקוח לשלוח שוב');
...
async function emailReport(office, clientName, profile, consentAt, photoSaved) {
  ...
  await transporter.sendMail({
    ...
    html: reportHtml(clientName, office, profile, consentAt, photoSaved)
  });
  return true;
}
...
async function submit(req, res) {
  ...
  // אם השמירה נכשלת (כללי אבטחה חוסמים תת-קולקציות וחשבון השירות
  // חסר הרשאה) לא מפילים את כל הטופס — שאר הפרטים חשובים לא פחות.
  // הכישלון מדווח החוצה כדי שאיש לא יחשוב שהצילום התקבל.
  ...
  if (idPhoto) {
    ...
    if (!photoSaved) {
      ...
      console.error('[onboarding] ID photo NOT saved:', r.status, detail.slice(0, 300),
        r.status === 403
          ? '— grant the service account the "Cloud Datastore User" role, or allow clients/{id}/{document=**} in firestore.rules'
          : '');
    }
  }
  ...
  let emailed = false;
  try { emailed = await emailReport(office, v.name, profile, now, photoSaved); }
  catch (e) { console.warn('onboarding report email failed:', e.message); }

  return res.status(200).json({ ok: true, emailed, photoSaved });
}
