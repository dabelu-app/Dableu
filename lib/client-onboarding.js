...
async function submit(req, res) {
  ...
  if (idPhoto) {
    const r = await fsFetch(`/clients/${cid}/documents?documentId=idCard`, {
      method: 'POST',
      body: JSON.stringify({ fields: toFields({ type: 'idCard', dataUrl: idPhoto, uploadedAt: now }) })
    });
    if (!r.ok && r.status === 409) {
      await fsFetch(`/clients/${cid}/documents/idCard`, {
        method: 'PATCH',
        body: JSON.stringify({ fields: toFields({ type: 'idCard', dataUrl: idPhoto, uploadedAt: now }) })
      });
    }
  ...
  const mask = ['profile', 'profileCompletedAt', 'privacyConsent', 'onboardingToken']
